...
/*   Updated: 2024/06/25 21:08:40 by ymunoz-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (valid == false)
		return (write(2, "Error\n", 6), free_all(&head_a, argv), 1);
	if (ft_is_ordered(head_a) == 0)
		return (free_all(&head_a, argv), 0);
	order_list(&head_a, &head_b);
	free_all(&head_a, argv);
	return (0);
}
